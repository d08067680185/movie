...
export default function AdminPage() {
  ...
  const [linkReports, setLinkReports] = useState<{ url: string; report_count: number; last_reported_at: string | null }[] | null>(null);
  ...
  async function loadLinkReports() {
    try {
      const res = await apiFetch("/api/admin/live-link-reports", {}, token);
      if (res.ok) setLinkReports(await res.json());
    } catch { setLinkReports(null); }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        {panHealth && (
          <div className="p-5 rounded-xl" style={{ background: DARK.bgCard, border: `1px solid ${panHealth.pansou === "up" ? "rgba(255,255,255,0.08)" : "rgba(248,113,113,0.5)"}` }}>
            ...
            <div className="mt-3">
              <button
                onClick={() => { const next = !showLinkReports; setShowLinkReports(next); if (next && !linkReports) loadLinkReports(); }}
                className="text-xs px-2 py-1 rounded"
                style={{ background: "rgba(255,255,255,0.06)", color: "#a0a0b0" }}
              >
                {showLinkReports ? "收起" : "查看"}用户失效举报榜单（众包标记，覆盖面比官方检测广）
              </button>
              {showLinkReports && (
                <div className="mt-2 max-h-72 overflow-y-auto rounded-lg" style={{ background: "rgba(255,255,255,0.03)" }}>
                  {!linkReports ? (
                    ...
                    <div className="p-3 text-xs" style={{ color: "#606070" }}>暂无举报数据</div>
                  ) : (
                    <table className="w-full text-xs">
                      <thead>
                        <tr style={{ color: "#606070" }}>
                          <th className="text-left py-1.5 px-3">链接</th>
                          <th className="text-right py-1.5 px-3">举报次数</th>
                          <th className="text-right py-1.5 px-3">最近举报</th>
                        </tr>
                      </thead>
                      <tbody>
                        {linkReports.map((r) => (
                          <tr key={r.url} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                            <td className="py-1.5 px-3 font-mono truncate max-w-xs" title={r.url}>{r.url}</td>
                            <td className="py-1.5 px-3 text-right" style={{ color: r.report_count >= 2 ? "#f87171" : "#f0f0f5" }}>
                              {r.report_count}
                            </td>
                            <td className="py-1.5 px-3 text-right" style={{ color: "#606070" }}>
                              {r.last_reported_at ? new Date(r.last_reported_at).toLocaleDateString("zh-CN") : "—"}
                            </td>
                          </tr>
                        ))}
                      </tbody>
