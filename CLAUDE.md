...
**全网搜(PanSou)补齐已写未接功能 + 数据质量 + 加固 (2026-08-16)**：
- 之前 `section`（板块加权）、`cloud_type`（服务端类型过滤）参数和 `hotWords`/`onPickHotWord` 空结果热词推荐早就在 `api/livesearch.py`/`LiveSearchResults.tsx` 里写好，但 `/search?mode=live` 页从没传过，等于白写。本轮补上：`SearchContent.tsx` 现在会 fetch `getHotSearches()` 传给 `LiveSearchResults`，并新增一行板块筛选 tabs（复用 `section` URL 参数 + `updateSearch`）。`cloud_type` 经核实是对**已缓存的完整结果**做二次过滤（不省上游调用也不影响缓存粒度），维持现状客户端本地过滤，未接线（接了反而会把现在瞬时的类型切换变成每次网络请求）。`/pan` 独立页保持无板块概念，不受影响（`LiveSearchResults` 的 `sections`/`section` 是可选 prop，不传就不渲染这行）。
- `_normalize()` 新增同网盘类型内的近似标题去重（复用 `dedup.py:clean_key()` 标题归一化），解决同一资源被不同 TG 频道转发产生不同 URL 但标题几乎一样的重复展示问题；不做跨类型去重（不同类型URL域名本身不同，不会重复），也不做跨类型资源卡片合并（更大的UI重设计，本轮不做）。
- `_clean_url` 现在把 url 字段的尾部 `#` 也一并清理（此前只清理 password 字段，理由是怕误伤 quark/aliyun 链接里的 SPA 路由片段如 `.../s/x#/list/share`——但 `rstrip` 只删字符串真正末尾字符，中间片段不受影响，这个担心不成立，已翻转并补测试锁定安全边界）。
- `LiveSearchResults.tsx` "最新"排序此前用 `.localeCompare` 比较 `datetime` 原始字符串，PanSou 偶发的占位日期（`0001-01-01`）会被字典序插到列表中间；改用真正的时间数值比较，无效/占位日期统一排最后。
- `/api/livesearch` 新增独立限流（30次/60秒/IP，参照 `/api/search` 的 `SlidingWindowLimiter` 范式，阈值更紧因为缓存未命中单次成本是10秒级上游调用）；`_fetch_pansou` 新增 `asyncio.Semaphore(4)` 限制不同关键词同时打上游的并发数（相同关键词已被 `_fetch_coalesced` 收敛，不受此信号量影响），因为 pansou 容器只有 1.0 cpu，单次搜索内部本身要并发查上百个源。
- **重新核实确认单worker部署**：`backend/Dockerfile` 的 `uvicorn main:app` 没带 `--workers`，默认单进程，`api/livesearch.py` 里"单worker部署；多worker需换Redis"的注释准确。**以后如果为了吞吐量给 Dockerfile 加 `--workers` 参数，必须同步把 `_cache`/`_inflight`/熔断状态/本轮新增的限流器和信号量都迁移到 Redis 或等价的跨进程共享存储，否则这些机制会悄悄退化成每个 worker 各自一份、互不感知**，不会报错但会静默失效。
