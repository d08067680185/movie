...
- `ratelimit.py:get_client_ip()` — always use this instead of raw `request.client.host`. It prefers the Cloudflare-set `CF-Connecting-IP` header, since the real deployment path is Cloudflare → (an Nginx layer outside this repo, config not found) → Docker, and `request.client.host` alone only ever sees the immediate upstream hop's address (not the real visitor). **Known residual risk**: nothing in this repo verifies the request actually came through Cloudflare, so a client that bypasses Cloudflare and hits the origin directly could in theory forge this header — that verification would need to happen in the Nginx layer, which isn't part of this codebase.
